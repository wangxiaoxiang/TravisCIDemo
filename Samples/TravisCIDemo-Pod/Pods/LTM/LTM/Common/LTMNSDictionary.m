...
static NSNumber *_LTMNSNumberFromID(id value) {
...
#define _LTM_RETURN_VALUE(_type_)                     \
    ...
    if ([value isKindOfClass:[NSString class]]) {     \
        NSNumber *num = _LTMNSNumberFromID(value);        \
        if (!num) return def;                         \
        return num._type_;                            \
    }                                                 \
    return def;
